<div class="my-container-home">
		  <div class="container-fluid">
			  <div class="row-fluid">
			  	<div class="span1">
			  	</div>
			    <div class="span3">
			      
			      <div><img class="my-img-home" src="<?php echo base_url();?>/assets/img/icon/MAESTROICON.png"><span>Chalala</span></div>
			      <div><img class="my-img-home" src="<?php echo base_url();?>/assets/img/icon/ESCUELAICON.png"><span>Chalala</span></div>
			      <div><img class="my-img-home" src="<?php echo base_url();?>/assets/img/icon/GRUPOICON.png"><span>Chalala</span></div>
			      <div><img class="my-img-home" src="<?php echo base_url();?>/assets/img/icon/ALUMNOICON.png"><span>Chalala</span></div>
			      <div><img class="my-img-home" src="<?php echo base_url();?>/assets/img/icon/HOMEICON.png"><span>Chalala</span></div>
			      <div><img class="my-img-home" src="<?php echo base_url();?>/assets/img/icon/MATERIAICON.png"><span>Chalala</span></div>

			    </div>
			    <div class="span7">
			      <div id="my-carousel" class="my-carousel">
			<div id="myCarousel" class="carousel slide ">
			  <!-- Carousel items -->
			  <div class="carousel-inner">

			    
			    <div class="active item">
			    	<img src="<?=base_url('assets/img/img1.jpg')?>">
			    	<div class="carousel-caption">
			    		<h4>Información</h4>
			    			<p>Más información, Más información, Más información, Más información...</p>
			    	</div>
			    </div>

			    <div class="item">
			    	<img src="<?=base_url('assets/img/img2.jpg')?>">
			    	<div class="carousel-caption">
			    		<h4>Información</h4>
			    			<p>Más información, Más información, Más información, Más información...</p>
			    	</div>
			    </div>

			    <div class="item">
					<img src="<?=base_url('assets/img/img3.jpg')?>">
					<div class="carousel-caption">
			    		<h4>Información</h4>
			    			<p>Más información, Más información, Más información, Más información...</p>
			    	</div>
			    </div>

			    <div class="item">
					<img src="<?=base_url('assets/img/img4.jpg')?>">
					<div class="carousel-caption">
			    		<h4>Información</h4>
			    			<p>Más información, Más información, Más información, Más información...</p>
			    	</div>
			    </div>

			    

			  </div>

			  <!-- Carousel nav -->
			  <a class="carousel-control left" href="#myCarousel" data-slide="prev">&lsaquo;</a>
			  <a class="carousel-control right" href="#myCarousel" data-slide="next">&rsaquo;</a>
			</div>
			
				

			</div>
		</div>
			    </div>
			  </div>
			</div>



		</div>
		<div class="span1"></div>
	</div>
	<div class="my-container-verde">
		
		
			<div class="row-fluid">
			  	<div class="span1">
			  	</div>
				<div class="span10">
			  <div>
			<img src="<?php echo base_url();?>/assets/img/icon/ESCUELA-BLANCOICON.png">
		      <span class="my-mapa-text">Mostrar Mapa</span>
		    </div>
					<iframe class="my-mapa" width="1100" height="500" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.com.mx/?ie=UTF8&amp;t=m&amp;ll=19.433572,-99.131699&amp;spn=0.323763,0.754623&amp;z=11&amp;output=embed"></iframe></div>
				</div>
				<div class="span1">
			  	</div>
		</div>